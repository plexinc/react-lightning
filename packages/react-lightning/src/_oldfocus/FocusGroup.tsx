...
export const FocusGroup = forwardRef<LightningElement, FocusGroupProps>(
  (
    ...
    const { focusPair, getFocusPath, removeFocusElement } =
      useContext(FocusPathContext);
    ...
    const autoFocusElements = useRef<Set<number>>(new Set());
    ...
    const handleFocusableChanged = useCallback(
      (element: LightningElement, focusable: boolean) => {
        // If the element is no longer focusable, and it's currently the focused element, we need to find another element to focus.
        if (!focusable && focusedChild.current === element) {
      ...
      [focusChild, getNextChild, updateHasFocusableChildren],
    );
    ...
    const handleFocusChanged = useCallback(
      (element: LightningElement, isFocused: boolean) => {
        if (!isFocused) {
          return;
        }
    ...
    const addChild = useCallback(
      (element: LightningElement, autoFocusChild?: boolean) => {
        if (
          (elementsRef.current.size === 0 ||
            (autoFocusElements.current.size === 0 && autoFocusChild)) &&
          element.visible
        ) {
          focusedChild.current = element;
        ...
        if (autoFocusChild) {
          autoFocusElements.current.add(element.id);
        }
        elementsRef.current.add(element);
        focusRef.current?.emit('focusChildAdded', element);

        const childDisposables = [
          element.on('focusableChanged', handleFocusableChanged),
          element.on('focusChanged', handleFocusChanged),
        ];
