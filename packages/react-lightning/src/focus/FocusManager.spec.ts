...
type FocusableElement = Focusable & {
  _focused: boolean;
  visible: boolean;
  name: string;
};

function createMockElement(name: string, visible = true): FocusableElement {
  return {
    _focused: false,
    name,
    visible,
...
describe('FocusManager', () => {
  let focusManager: FocusManager<FocusableElement>;
  ...
  it('should blur the previously focused element when a new one is focused', () => {
    const element1 = createMockElement('a');
    const element2 = createMockElement('b');
  ...
  it('should not focus an element if it is not visible', () => {
    const element = createMockElement('a', false);
    focusManager.addElement(element, null, { autoFocus: true });
  ...
  it('should recalculate the focus path when another element is focused in the tree', () => {
    const grandParent1 = createMockElement('grandParent1');
    const grandParent2 = createMockElement('grandParent2');
    const parent1 = createMockElement('parent1');
    const parent2 = createMockElement('parent2');
    const child1 = createMockElement('child1');
    const child2 = createMockElement('child2');
    const child3 = createMockElement('child3');
    const child4 = createMockElement('child4');
  ...
  it('should remove child elements from focus when parent is removed', () => {
    const grandParent = createMockElement('grandParent');
    const parent = createMockElement('parent');
    const child = createMockElement('child');
