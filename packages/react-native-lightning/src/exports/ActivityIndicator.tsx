...
export type ActivityIndicatorProps = RNActivityIndicatorProps & RefAttributes<LightningViewElement>;

export const ActivityIndicator: ForwardRefExoticComponent<ActivityIndicatorProps> = forwardRef<
  LightningViewElement,
  ActivityIndicatorProps
>(({ color, size }, ref) => {
  const duration = 1200;
  const [rotation, setRotation] = useState(0);
  const actualColor = htmlColorToLightningColor((color as string) || 'lightblue');
